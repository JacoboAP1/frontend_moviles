package com.proyecto_moviles.oficiar.auth;
...
public class SecurityConfig {
    ...
    /**
     * Constructor explícito para inyectar las dependencias necesarias.
     * Reemplaza la anotación @RequiredArgsConstructor para asegurar que IntelliJ reconozca
     * los beans inyectados sin marcar error en setUserDetailsService.
     */
    public SecurityConfig(JwtAuthFilter jwtFilter, UserDetailsService uds, PasswordEncoder encoder) {
    ...
    @Bean
    AuthenticationProvider authenticationProvider() {
        DaoAuthenticationProvider provider = new DaoAuthenticationProvider(uds); // Se pasa 'uds' directamente en el constructor
